...
### 📚 Documentation Console Upgrade
- **Homepage**: Rewritten as "Documentation Console" entry point with 6-section structure.
- **README**: Remediated to protocol-grade entry console format (normative truth boundary, quickstart, canonical URLs).
- **Sidebar**: Fully autogenerated from `_category_.json` files for consistent category-index linking.
- **Styling**: MPLP brand fonts (Inter, JetBrains Mono), compact typography, light mode default.
- **Footer**: Updated links to category index pages, copyright aligned with mplp.io.
- **URLs**: Canonical entry points unified (mplp.io, docs.mplp.io, npm, PyPI).
