# MPLP Contributors

This file lists all the people who have contributed to the MPLP (Multi-Agent Protocol Lifecycle Platform) project.

## 🏆 Core Team

The core team members who have made significant contributions to the project:

### **Project Lead**
- **Coregentis Team** - Project architecture, core development, and overall direction

## 👥 Contributors

We thank all contributors who have helped make MPLP better:

### **Code Contributors**

Contributors who have submitted code, bug fixes, or features:

- **Coregentis Team** - Core platform development, all 10 modules, SDK ecosystem

### **Documentation Contributors**

Contributors who have improved documentation:

- **Coregentis Team** - Complete documentation suite (English, Chinese, Japanese)

### **Community Contributors**

Contributors who have helped with community building, support, and advocacy:

- **Early Adopters** - Testing, feedback, and bug reports

## 🌟 Special Thanks

Special thanks to:

- **All Beta Testers** - For testing MPLP v1.1.0-beta and providing valuable feedback
- **Community Members** - For discussions, questions, and suggestions
- **Security Researchers** - For responsible disclosure of security issues

## 📝 How to Contribute

We welcome contributions from everyone! Here's how you can contribute:

1. **Code Contributions**: Submit pull requests for bug fixes or new features
2. **Documentation**: Improve or translate documentation
3. **Bug Reports**: Report bugs through GitHub Issues
4. **Feature Requests**: Suggest new features or improvements
5. **Community Support**: Help other users in discussions

See our [Contributing Guide](./CONTRIBUTING.md) for more details.

## 🏅 Contribution Recognition

Contributors are recognized in the following ways:

- **This File**: All contributors are listed here
- **CHANGELOG.md**: Notable contributions are mentioned in release notes
- **GitHub**: Contributor badge on your profile
- **Release Notes**: Major contributions are highlighted in releases

## 📊 Contribution Statistics

### **MPLP v1.1.0-beta**
- **Total Commits**: 1,000+
- **Total Contributors**: Growing community
- **Lines of Code**: 50,000+
- **Test Coverage**: 100% (2,905/2,905 tests passing)
- **Documentation Pages**: 200+

### **Project Milestones**
- ✅ **v1.0 Alpha**: Complete L1-L3 Protocol Stack (September 2025)
- ✅ **v1.1.0-beta**: Complete SDK Ecosystem (September 2025)
- 🚀 **v1.2.0**: Planned for Q4 2025

## 🌍 Global Community

MPLP has contributors and users from around the world:

- **Languages**: English, Chinese (Simplified), Japanese, and more
- **Countries**: Global presence across multiple continents
- **Industries**: Technology, Finance, Healthcare, Education, and more

## 💬 Join Our Community

- **GitHub Discussions**: [Join the conversation](https://github.com/Coregentis/MPLP-Protocol/discussions)
- **GitHub Issues**: [Report bugs or request features](https://github.com/Coregentis/MPLP-Protocol/issues)
- **Documentation**: [Read the docs](./docs/README.md)

## 📄 License

All contributions to MPLP are made under the [MIT License](./LICENSE).

---

**Last Updated**: October 17, 2025

**Want to see your name here?** Check out our [Contributing Guide](./CONTRIBUTING.md) and start contributing today!

Thank you to all our contributors! 🎉

