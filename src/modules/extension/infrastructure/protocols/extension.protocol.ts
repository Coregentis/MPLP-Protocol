 ...
 * Extension协议实现类
 */
export class ExtensionProtocol implements IMLPPProtocol {
  private readonly config: ExtensionProtocolConfig;
  private readonly dependencies: ExtensionProtocolDependencies;
  private readonly adapter: ExtensionModuleAdapter;
  private isInitialized: boolean = false;

  constructor(
    config: ExtensionProtocolConfig,
   ...
   * 处理协议请求
   */
  async handleRequest(request: MLPPRequest): Promise<MLPPResponse> {
    if (!this.isInitialized) {
    ...
    try {
      switch (request.operation) {
        case 'create_extension': {
          const result = await this.adapter.createExtension(request.payload as unknown as import('../adapters/extension-module.adapter').CreateExtensionRequest);
          return {
        ...
        case 'get_extension': {
          const result = await this.adapter.getExtension(request.payload.extensionId as UUID);
          return {
            protocolVersion: request.protocolVersion,
            timestamp,
            requestId: request.requestId,
        ...
        case 'update_extension': {
          const result = await this.adapter.updateExtension(
            request.payload.extensionId as UUID,
            request.payload.updates as Partial<ExtensionEntityData>
          );
          return {
            protocolVersion: request.protocolVersion,
            timestamp,
        ...
        case 'delete_extension': {
          const result = await this.adapter.deleteExtension(request.payload.extensionId as UUID);
          return {
            protocolVersion: request.protocolVersion,
            timestamp,
            requestId: request.requestId,
        ...
        case 'activate_extension': {
          const result = await this.adapter.activateExtension(request.payload.extensionId as UUID);
          return {
            protocolVersion: request.protocolVersion,
            timestamp,
            requestId: request.requestId,
        ...
        case 'deactivate_extension': {
          const result = await this.adapter.deactivateExtension(request.payload.extensionId as UUID);
          return {
            protocolVersion: request.protocolVersion,
            timestamp,
            requestId: request.requestId,
        ...
        case 'list_extensions': {
          const result = await this.adapter.listExtensions(request.payload as unknown as import('../adapters/extension-module.adapter').ExtensionQueryOptions);
          return {
            protocolVersion: request.protocolVersion,
            timestamp,
            requestId: request.requestId,
        ...
        case 'query_extensions': {
          const result = await this.adapter.queryExtensions(request.payload as unknown as import('../adapters/extension-module.adapter').ExtensionQueryCriteria);
          return {
            protocolVersion: request.protocolVersion,
            timestamp,
            requestId: request.requestId,
        ...
        case 'get_active_extensions': {
          const result = await this.adapter.getActiveExtensions(request.payload?.contextId as UUID);
          return {
            protocolVersion: request.protocolVersion,
            timestamp,
            requestId: request.requestId,
        ...
        case 'get_extensions_by_type': {
          const result = await this.adapter.getExtensionsByType(request.payload.extensionType as ExtensionType);
          return {
            protocolVersion: request.protocolVersion,
            timestamp,
            requestId: request.requestId,
   ...
   * 初始化L3管理器（预留接口）
   */
  private async initializeL3Managers(): Promise<void> {
   ...
   * 关闭L3管理器
   */
  private async shutdownL3Managers(): Promise<void> {
    // TODO: 关闭L3管理器
