...
export class ExtensionMapper {
  ...
  static validateSchema(data: unknown): ExtensionSchema {
    ...
    if (!validExtensionTypes.includes(schema.extension_type as string)) {
    ...
    return schema as unknown as ExtensionSchema;
  }
   ...
   * 兼容性信息映射 - 转换为Schema
   */
  private static compatibilityToSchema(compatibility: ExtensionCompatibility): ExtensionCompatibilitySchema {
    return {
   ...
   * 兼容性信息映射 - 从Schema转换
   */
  private static compatibilityFromSchema(schema: ExtensionCompatibilitySchema): ExtensionCompatibility {
    return {
   ...
   * 配置信息映射 - 转换为Schema
   */
  private static configurationToSchema(configuration: ExtensionConfiguration): ExtensionConfigurationSchema {
    return {
      ...
      validation_rules: configuration.validationRules.map(rule => ({
        ...
        severity: rule.required ? 'error' as const : 'warning' as const
      }))
    };
  }
   ...
   * 配置信息映射 - 从Schema转换
   */
  private static configurationFromSchema(schema: ExtensionConfigurationSchema): ExtensionConfiguration {
    return {
   ...
   * 扩展点映射 - 转换为Schema
   */
  private static extensionPointsToSchema(points: ExtensionPoint[]): ExtensionPointSchema[] {
   ...
   * 扩展点映射 - 从Schema转换
   */
  private static extensionPointsFromSchema(schemas: ExtensionPointSchema[]): ExtensionPoint[] {
   ...
   * API扩展映射 - 转换为Schema
   */
  private static apiExtensionsToSchema(extensions: ApiExtension[]): ApiExtensionSchema[] {
    return extensions.map(ext => ({
      endpoint_id: `api-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`,
      path: ext.endpoint,
      method: ext.method.toLowerCase() as 'get' | 'post' | 'put' | 'patch' | 'delete' | 'options' | 'head',
   ...
   * API扩展映射 - 从Schema转换
   */
  private static apiExtensionsFromSchema(schemas: ApiExtensionSchema[]): ApiExtension[] {
    return schemas.map(schema => ({
      endpoint: schema.path,
      method: schema.method.toUpperCase() as 'GET' | 'POST' | 'PUT' | 'DELETE' | 'PATCH' | 'HEAD' | 'OPTIONS',
   ...
   * 事件订阅映射 - 转换为Schema
   */
  private static eventSubscriptionsToSchema(subscriptions: EventSubscription[]): EventSubscriptionSchema[] {
   ...
   * 事件订阅映射 - 从Schema转换
   */
  private static eventSubscriptionsFromSchema(schemas: EventSubscriptionSchema[]): EventSubscription[] {
    return schemas.map(schema => ({
      ...
      filterConditions: Object.entries(schema.filter_conditions || {}).map(([field, condition]) => ({
        ...
        operator: (typeof condition === 'object' && condition !== null && 'operator' in condition
          ? condition.operator as 'eq' | 'ne' | 'gt' | 'gte' | 'lt' | 'lte' | 'in' | 'nin' | 'regex'
          : 'eq'),
   ...
   * 生命周期映射 - 转换为Schema
   */
  private static lifecycleToSchema(lifecycle: ExtensionLifecycle): ExtensionLifecycleSchema {
    return {
   ...
   * 生命周期映射 - 从Schema转换
   */
  private static lifecycleFromSchema(schema: ExtensionLifecycleSchema): ExtensionLifecycle {
    return {
   ...
   * 安全配置映射 - 转换为Schema
   */
  private static securityToSchema(security: ExtensionSecurity): ExtensionSecuritySchema {
    return {
      ...
      permissions: Object.entries(security.permissions).flatMap(([category, perms]: [string, unknown]) => {
   ...
   * 安全配置映射 - 从Schema转换
   */
  private static securityFromSchema(schema: ExtensionSecuritySchema): ExtensionSecurity {
    return {
   ...
   * 元数据映射 - 转换为Schema
   */
  private static metadataToSchema(metadata: ExtensionMetadata): ExtensionMetadataSchema {
    return {
   ...
   * 元数据映射 - 从Schema转换
   */
  private static metadataFromSchema(schema: ExtensionMetadataSchema): ExtensionMetadata {
    return {
      author: {
   ...
   * 审计跟踪映射 - 转换为Schema
   */
  private static auditTrailToSchema(auditTrail: AuditTrail): AuditTrailSchema {
    return {
      enabled: true,
  ...
  private static auditTrailFromSchema(schema: AuditTrailSchema): AuditTrail {
    return {
      events: schema.audit_events?.map(event => ({
        id: event.event_id,
        timestamp: event.timestamp,
        eventType: event.event_type as 'install' | 'uninstall' | 'activate' | 'deactivate' | 'configure' | 'update' | 'error' | 'security_violation',
   ...
   * 性能指标映射 - 转换为Schema
   */
  private static performanceMetricsToSchema(metrics: ExtensionPerformanceMetrics): ExtensionPerformanceMetricsSchema {
    return {
      enabled: true,
   ...
   * 性能指标映射 - 从Schema转换
   */
  private static performanceMetricsFromSchema(schema: ExtensionPerformanceMetricsSchema): ExtensionPerformanceMetrics {
    return {
   ...
   * 监控集成映射 - 转换为Schema
   */
  private static monitoringIntegrationToSchema(integration: MonitoringIntegration): MonitoringIntegrationSchema {
    return {
   ...
   * 版本历史映射 - 转换为Schema
   */
  private static versionHistoryToSchema(history: VersionHistory): VersionHistorySchema {
    return {
      enabled: true,
      max_versions: 10,
   ...
   * 版本历史映射 - 从Schema转换
   */
  private static versionHistoryFromSchema(schema: VersionHistorySchema): VersionHistory {
    return {
   ...
   * 搜索元数据映射 - 转换为Schema
   */
  private static searchMetadataToSchema(metadata: SearchMetadata): SearchMetadataSchema {
    return {
      enabled: true,
   ...
   * 搜索元数据映射 - 从Schema转换
   */
  private static searchMetadataFromSchema(schema: SearchMetadataSchema): SearchMetadata {
    return {
   ...
   * 事件集成映射 - 转换为Schema
   */
  private static eventIntegrationToSchema(integration: EventIntegration): EventIntegrationSchema {
    return {
      enabled: true,
   ...
   * 事件集成映射 - 从Schema转换
   */
  private static eventIntegrationFromSchema(schema: EventIntegrationSchema): EventIntegration {
    return {
      eventBus: {
