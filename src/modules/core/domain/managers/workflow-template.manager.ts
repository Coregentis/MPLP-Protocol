...
import { WorkflowConfig, WorkflowStageType, ExecutionModeType, UUID, ExecutionMode, Priority } from '../../types';
...
export class WorkflowTemplateManager {
  private readonly customTemplates = new Map<UUID, WorkflowTemplate>();
  private readonly templateUsageStats = new Map<UUID, TemplateUsageStats>();
   ...
   * 更新自定义工作流模板
   */
  async updateCustomTemplate(templateId: UUID, updates: Partial<CustomTemplateData>): Promise<WorkflowTemplate> {
   ...
   * 删除自定义工作流模板
   */
  async deleteCustomTemplate(templateId: UUID): Promise<boolean> {
    ...
    return allTemplates
      .filter(template => {
        ...
        if (context.complexity) {
          const templateComplexity = template.metadata?.complexity as string;
        ...
        if (context.resourceConstraints) {
          const resourceReq = template.metadata?.resourceRequirements as string;
   ...
   * 验证模板数据
   */
  private async validateTemplateData(templateData: CustomTemplateData): Promise<TemplateValidationResult> {
    const errors: string[] = [];
    ...
    // 验证执行模式
    const validExecutionModes: ExecutionMode[] = [ExecutionMode.SEQUENTIAL, ExecutionMode.PARALLEL, ExecutionMode.CONDITIONAL];
   ...
   * 生成模板ID
   */
  private generateTemplateId(): UUID {
   ...
   * 记录模板使用
   */
  private recordTemplateUsage(templateId: UUID): void {
    const stats = this.templateUsageStats.get(templateId);
    if (stats) {
      stats.usageCount++;
