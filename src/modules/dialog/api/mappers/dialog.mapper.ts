...
import {
  DialogSchema,
  DialogCapabilitiesSchema,
  DialogStrategySchema,
  DialogContextSchema,
  DialogConfigurationSchema,
  AuditTrailSchema,
  AuditEventSchema,
...
} from '../../domain/entities/dialog.entity';
...
export class DialogMapper {
  ...
  static toSchema(entity: DialogEntity): DialogSchema {
    return {
      ...
      audit_trail: {
        ...
        audit_events: entity.auditTrail.auditEvents?.map((event: AuditEvent) => ({
      ...
      performance_metrics: {
        ...
        health_status: entity.performanceMetrics.healthStatus ? {
          ...
          checks: entity.performanceMetrics.healthStatus.checks?.map((check: HealthCheck) => ({
      ...
      version_history: {
        ...
        versions: entity.versionHistory.versions?.map((version: Version) => ({
      ...
      search_metadata: {
        ...
        search_indexes: entity.searchMetadata.searchIndexes?.map((index: SearchIndex) => ({
      ...
      event_integration: {
        ...
        event_routing: entity.eventIntegration.eventRouting ? {
          routing_rules: entity.eventIntegration.eventRouting.routingRules?.map((rule: EventRoutingRule) => ({
            rule_id: rule.ruleId,
  ...
  static fromSchema(schema: DialogSchema): DialogEntity {
    return new DialogEntity(
      ...
      schema.name,
      schema.participants,
      this.capabilitiesFromSchema(schema.capabilities),
      this.auditTrailFromSchema(schema.audit_trail),
      this.monitoringFromSchema(schema.monitoring_integration),
      this.performanceFromSchema(schema.performance_metrics),
      this.versionHistoryFromSchema(schema.version_history),
      this.searchMetadataFromSchema(schema.search_metadata),
      schema.dialog_operation,
      this.eventIntegrationFromSchema(schema.event_integration),
      schema.protocol_version,
      schema.timestamp,
      schema.description,
      schema.strategy ? this.strategyFromSchema(schema.strategy) : undefined,
      schema.context ? this.contextFromSchema(schema.context) : undefined,
      this.configurationFromSchema(schema.configuration),
      schema.metadata,
      schema.dialog_details ? this.dialogDetailsFromSchema(schema.dialog_details) : undefined
    );
  }

  private static auditTrailFromSchema(auditTrail: AuditTrailSchema): AuditTrail {
    return {
      enabled: auditTrail.enabled,
      retentionDays: auditTrail.retention_days,
      auditEvents: auditTrail.audit_events?.map((event: AuditEventSchema) => ({
  ...
  private static monitoringFromSchema(monitoring: MonitoringIntegrationSchema): MonitoringIntegration {
    return {
  ...
  private static performanceFromSchema(performance: PerformanceMetricsSchema): PerformanceMetrics {
    return {
  ...
  private static versionHistoryFromSchema(versionHistory: VersionHistorySchema): VersionHistory {
    return {
      enabled: versionHistory.enabled,
      maxVersions: versionHistory.max_versions,
      versions: versionHistory.versions?.map((version: VersionSchema) => ({
  ...
  private static searchMetadataFromSchema(searchMetadata: SearchMetadataSchema): SearchMetadata {
    return {
      ...
      searchIndexes: searchMetadata.search_indexes?.map((index: SearchIndexSchema) => ({
  ...
  private static eventIntegrationFromSchema(eventIntegration: EventIntegrationSchema): EventIntegration {
    return {
      ...
      eventRouting: eventIntegration.event_routing ? {
        routingRules: eventIntegration.event_routing.routing_rules?.map((rule: EventRoutingRuleSchema) => ({
          ruleId: rule.rule_id,
          condition: rule.condition,
  ...
  private static configurationFromSchema(configuration?: DialogConfigurationSchema): DialogConfiguration | undefined {
  ...
  private static dialogDetailsFromSchema(dialogDetails: DialogDetailsSchema): DialogDetails {
    return {
  ...
  static validateSchema(schema: DialogSchema): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];

    // 验证必需字段
    if (!schema.protocol_version) errors.push('protocol_version is required');
    if (!schema.timestamp) errors.push('timestamp is required');
    if (!schema.dialog_id) errors.push('dialog_id is required');
    if (!schema.name) errors.push('name is required');
    if (schema.name && schema.name.length > 255) errors.push('name must be 255 characters or less');
    if (!schema.participants || schema.participants.length === 0) errors.push('at least one participant is required');
    if (!schema.capabilities) errors.push('capabilities is required');
    if (!schema.audit_trail) errors.push('audit_trail is required');
    if (!schema.monitoring_integration) errors.push('monitoring_integration is required');
    if (!schema.performance_metrics) errors.push('performance_metrics is required');
    if (!schema.version_history) errors.push('version_history is required');
    if (!schema.search_metadata) errors.push('search_metadata is required');
    if (!schema.dialog_operation) errors.push('dialog_operation is required');
    if (!schema.event_integration) errors.push('event_integration is required');
  ...
  // 私有辅助方法
  private static capabilitiesToSchema(capabilities: DialogCapabilities): DialogCapabilitiesSchema {
    return {
      basic: {
  ...
  private static capabilitiesFromSchema(schema: DialogCapabilitiesSchema): DialogCapabilities {
    return {
      basic: {
  ...
  private static strategyToSchema(strategy: DialogStrategy): DialogStrategySchema {
    return {
      type: strategy.type,
  ...
  private static strategyFromSchema(schema: DialogStrategySchema): DialogStrategy {
    return {
      type: schema.type,
  ...
  private static contextToSchema(context: DialogContext): DialogContextSchema {
    return {
  ...
  private static contextFromSchema(schema: DialogContextSchema): DialogContext {
    return {
