...
describe('ConfirmProtocol修复测试', () => {
  ...
  describe('healthCheck方法测试', () => {
    it('应该返回健康状态', async () => {
      ...
      expect(typeof health.timestamp).toBe('string'); // 时间戳是字符串格式
  ...
  describe('协议标准符合性测试', () => {
    ...
    it('executeOperation方法应该返回标准MLPPResponse', async () => {
      const createRequest = createMockCreateConfirmRequest();
      mockConfirmService.createConfirm.mockResolvedValue(mockConfirmData);
  ...
  describe('MPLP协议接口测试', () => {
    ...
    it('应该支持所有标准操作', () => {
      const metadata = confirmProtocol.getMetadata();
      const expectedOperations = ['create', 'get', 'list', 'query', 'update', 'approve', 'reject', 'delete'];
  ...
  describe('错误处理测试', () => {
    it('应该正确处理无效payload', async () => {
      // 设置Mock Service在接收null payload时抛出错误
      mockConfirmService.createConfirm.mockRejectedValue(new Error('Invalid payload'));
