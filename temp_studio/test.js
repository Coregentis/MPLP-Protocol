...
async function testNewComponents() {
  ...
  try {
    ...
    // 5. 测试StudioServer (不启动实际服务器)
    console.log('\n5. 测试StudioServer...');
    const studioServer = new StudioServer(studioApp.getConfig(), studioApp.getEventManager(), studioApp);
    console.log('✅ StudioServer创建成功，状态:', studioServer.getStatus());
    ...
    console.log('   ✅ StudioServer: 创建、状态检查、关闭 - 全部成功');
