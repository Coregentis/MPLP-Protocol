...
doc_type: normative
trace:
  schema: schemas/v2/modules/mplp-plan.schema.json
  tests: packages/sources/sdk-ts/__tests__/plan.test.ts
title: Plan Module
description: Plan Module specification for MPLP task decomposition. Defines the Plan object as a Directed Acyclic Graph (DAG) of executable steps with dependency management.
keywords: [MPLP, Multi-Agent Lifecycle Protocol, Agent OS Protocol, AI Agent, Observable, Governed, Vendor-neutral, Plan Module, task decomposition, DAG execution, dependency graph, MPLP plan, step execution, agent planning]
