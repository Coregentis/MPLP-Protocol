...
description: Context Module specification for MPLP project state. Defines the
  root anchor for Plans, Traces, and Roles, encapsulating domain, environment,
  and constraints.
keywords:
  - Context Module
  - project state
...
spec_level: L2
normative_id: MPLP-MOD-CONTEXT
permalink: /modules/context-module
modules:
  - context
normative_refs: []
protocol_alignment:
  ...
  protocol_version: 1.0.0
  schema_refs:
    - schema_id: https://schemas.mplp.dev/v1.0/mplp-context.schema.json
      local_path: packages/sdk-ts/schemas/mplp-context.schema.json
...
```
Context Module  Plan Module (context_id binding)        Every Plan MUST reference valid context_id  Role Module (owner_role reference)        owner_role links to role_id  Trace Module (trace binding)        Global trace for project-level observability  Confirm Module (governance.lastConfirmRef)
             Last approval decision reference
```
...
**Modules**:
- [Plan Module](plan-module.md) - Plans bind to Context
- [Role Module](role-module.md) - owner_role reference
- [Trace Module](trace-module.md) - Global trace binding

**Cross-Cutting**:
- [State Sync](../01-architecture/cross-cutting/state-sync.md) - Context synchronization
