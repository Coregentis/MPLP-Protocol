...
description: Matrix of data flow paths between MPLP L2 Modules and the Project
  Semantic Graph (PSG). Defines access modes (READ/WRITE) and ownership for each
  module.
keywords:
  - Module PSG Paths
  - PSG data flow
...
protocol_alignment:
  ...
  schema_refs:
    - schema_id: https://schemas.mplp.dev/v1.0/mplp-context.schema.json
      binding: manual
  invariant_refs:
    - local_path: tests/golden/invariants/context.yaml
      binding: manual
  golden_refs: []
  code_refs:
    ts: []
...
normative_id: MPLP-RT-MODULE-PSG-PATHS
sidebar_position: 2
---
> [!FROZEN]
