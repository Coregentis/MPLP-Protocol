...
description: Value State Layer specification for MPLP runtime. Covers state persistence, pluggable backends, snapshot/restore, transaction support, and encryption at rest.
keywords: [MPLP, Multi-Agent Lifecycle Protocol, Agent OS Protocol, AI Agent, Observable, Governed, Vendor-neutral, VSL, Value State Layer, state persistence, PSG storage, snapshots, transactions, Redis, PostgreSQL, encryption]
sidebar_label: VSL
---
> [!FROZEN]
...
- Guarantee consistency for concurrent access
...
### 2.3 Production Interface (Extended)
...
**Why**: Events are immutable historical recordsppend-only ensures integrity
...
**Immutability Invariant**: Once appended, events **MUST NOT** be modified

### 3.3 Read-After-Write Consistency

**Requirement**: The VSL **MUST** guarantee Read-after-Write consistency for the Orchestrator
...
### 6.2 Acknowledged Write Durability
...
### 8.1 Snapshot/Restore (for Rollback)
...
### 8.2 Transaction Support (ACID VSL)
...
- [L1-L4 Deep Dive](../01-architecture/l1-l4-architecture-deep-dive.md) (VSLExtended interface)
