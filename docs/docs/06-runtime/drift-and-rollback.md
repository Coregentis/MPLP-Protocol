...
description: Specification for Drift Detection and Rollback mechanisms in MPLP
  runtimes. Defines strategies for detecting state divergence and restoring PSG
  consistency.
keywords:
  - Drift Detection
  - Rollback
...
protocol_alignment:
  ...
  schema_refs:
    - schema_id: https://schemas.mplp.dev/v1.0/mplp-trace.schema.json
      binding: manual
    - schema_id: https://schemas.mplp.dev/v1.0/mplp-context.schema.json
      binding: manual
  invariant_refs:
    - local_path: schemas/invariants/runtime-drift-invariants.yaml
      binding: manual
  golden_refs: []
  code_refs:
    ts: []
...
normative_id: MPLP-RT-DRIFT-AND-ROLLBACK
sidebar_position: 4
---
> [!FROZEN]
...
- **state-sync**: PSG consistency guarantee
...
**Cross-Cutting**:
- [Error Handling](../01-architecture/cross-cutting/error-handling.md)
