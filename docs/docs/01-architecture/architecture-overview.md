...
|:----------|:-----------|
| **Implement the spec** | [L1 Core Protocol](./l1-core-protocol.md) |
| **Understand orchestration** | [L3 Execution Layer](./l3-execution-orchestration.md) |
| **Verify compliance** | [Golden Flows](/docs/golden-flows) |
| **Explore modules** | [Module Specifications](/docs/modules/core-module) |

## 2. Design Principles

MPLP's architecture is founded on these non-destructive protocol invariants:

1.  **Schema-First Design**: All protocol entities are rigorously defined using JSON Schema Draft-07 specifications located in `schemas/v2/` (29 schemas total). Schemas serve as the normative contract. Implementations MUST validate all data against these schemas using conformant validators.
...
**Status**: NORMATIVE - Required for all conformant implementations  
**Location**: `schemas/v2/`  
**SDK Implementation**: `packages/sdk-ts/src/core/`, validation via AJV v8.12.0

### 4.1 Responsibility

L1 defines the "physics" of the protocol – the invariant data structures and validation rules that all higher layers must obey. It is purely **declarative** and **final** once frozen.
...
2. Validate ALL input/output against schemas using JSON Schema Draft-07 conformant validator
...
L4 connects external systems (IDE, Git, CI/CD, tools) to the MPLP runtime. Adapters translate external events into L1-conformant integration events.
...
<div className="card-grid">
  <PathCard
    ...
    description="Deep dive into the normative schemas."
    to="/docs/architecture/l1-core-protocol"
    icon="📜"
  />
  <PathCard
    title="L3 Execution"
    description="Learn how runtimes execute plans."
    to="/docs/architecture/l3-execution-orchestration"
    icon="⚙️"
  />
</div>
