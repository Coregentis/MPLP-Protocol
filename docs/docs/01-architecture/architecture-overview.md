...
description: Comprehensive architectural view of MPLP v1.0 four-layer
  architecture (L1-L4), design principles, normative boundaries, and
  implementation guidance for multi-agent AI systems.
keywords:
  - MPLP architecture
  - L1 L2 L3 L4
  - protocol layers
...
spec_level: L1
normative_id: MPLP-ARCH-OVERVIEW
...
## 8. Cross-Cutting Concerns (15 Aspects)

MPLP defines concerns that span all layers. Based on actual files in `docs/01-architecture/cross-cutting/` (15 files):

### 8.1 Core 9 Concerns (from `metadata.schema.json`)
These 9 concerns are enumerated in `metadata.cross_cutting[]`:

1. **coordination** - Multi-agent handoffs and turn-taking
2. **error-handling** - Failure detection, recovery, retry
3. **event-bus** - Event routing and dispatch
4. **orchestration** - Plan step sequencing, dependency resolution
5. **performance** - Latency, throughput, token cost tracking
6. **protocol-version** - Compatibility checks, migration paths
7. **security** - Access control, data safety, role-based permissions
8. **state-sync** - PSG consistency across distributed components
9. **transaction** - Atomicity, rollback, compensation

### 8.2 Extended Runtime Concerns (6 Additional)
Additional concerns documented in cross-cutting/:

10. **AEL** (`ael.md`) - Action Execution Layer patterns
11. **VSL** (`vsl.md`) - Value State Layer patterns
12. **Observability** (`observability.md`) - 12 event families, structured logging
13. **Learning Feedback** (`learning-feedback.md`) - Learning loop integration
14. **Protocol Versioning** (`protocol-versioning.md`) - Detailed versioning strategy
15. **Overview** (`overview.md`) - Cross-cutting architecture summary

**Normative Rule**: Modules MAY opt-in to concerns via `meta.cross_cutting[]` array.
...
### 12.2 Python SDK (`mplp-sdk` v1.0.0)
...
### 13.1 Layer Specifications
- **L1 Details**: [L1 Core Protocol](l1-core-protocol.md)
- **L2 Details**: [L2 Coordination & Governance](l2-coordination-governance.md)
- **L3 Details**: [L3 Execution & Orchestration](l3-execution-orchestration.md)
- **L4 Details**: [L4 Integration Infrastructure](l4-integration-infra.md)

### 13.2 Deep Dives
- **Architecture Deep Dive**: [L1-L4 Architecture Deep Dive](l1-l4-architecture-deep-dive.md)
- **Schema Conventions**: [Schema Conventions](schema-conventions.md)
- **Cross-Cutting**: [Cross-Cutting Concerns Overview](cross-cutting/overview.md)

### 13.3 Implementation Guides
- **Module Specifications**: [context-module.md](../02-modules/context-module.md) (10 module docs)
- **Execution Profiles**: [sa-profile.md](../03-profiles/sa-profile.md) (SA/MAP specs)
- **Compliance**: [Compliance Guide](../08-guides/mplp-v1.0-compliance-guide.md)
- **SDK Guides**: [TypeScript SDK](../10-sdk/ts-sdk-guide.md), [Python SDK](../10-sdk/py-sdk-guide.md)
