...
description: Technical deep dive into MPLP four-layer architecture covering
  AEL/VSL internals, PSG algorithms, MAP coordination protocols, drift
  detection, and advanced security considerations.
keywords:
  - MPLP deep dive
  - AEL
  - VSL
...
normative_id: MPLP-L1L4-DEEPDIVE
permalink: /architecture/l1-l4-architecture-deep-dive
normative_refs: []
protocol_alignment:
  truth_level: T2
  protocol_version: 1.0.0
...
This document provides a **comprehensive technical deep dive** into the MPLP v1.0 four-layer architecture, focusing on advanced topics, internal mechanics, and complex inter-layer interactions. It is intended for:
- **Runtime Architects**: Designing MPLP-compliant runtimes
- **Advanced Implementers**: Building production-grade systems
- **Protocol Contributors**: Extending MPLP with custom profiles

**In Scope**:
- AEL/VSL internal mechanics and reference implementations
- PSG state transition algorithms and graph integrity
- MAP coordination protocols and turn dispatch
- Drift detection detailed algorithms
- Cross-layer binding points and contracts
- Advanced security and safety considerations

**Out of Scope**:
- Basic API usage (see L1-L4 individual specs)
- SDK quickstart guides (see `packages/sdk-ts/README.md`)
- Simple examples (see Golden Flows in `tests/golden/flows/`)

## 2. Architecture Review (Quick Reference)

**4-Layer Structure**:

| Layer | Status | Codebase Location | Lines of Code (TS SDK) |
|:---|:---|:---|---:|
| **L4: Integration** | Optional | `schemas/v2/integration/` (schemas only) | 0 (no runtime code) |
| **L3: Runtime** | Behavioral | `packages/sdk-ts/src/runtime-minimal/` | 75 |
| **L2: Modules** | Normative | `packages/sdk-ts/src/coordination/` | 12 |
| **L1: Core** | Normative | `packages/sdk-ts/src/core/` | 36 |

**Key Principle**: Lower layers are oblivious to higher layers (L1 doesn't know about L3)
...
**Reference Interface** (`packages/sdk-ts/src/runtime-minimal/index.ts` lines 29-31):
```typescript
...
**Reference Interface** (`packages/sdk-ts/src/runtime-minimal/index.ts` lines 24-27):
```typescript
...
**PSG Operations**:
From `docs/06-runtime/module-psg-paths.md`:
...
- [PSG Binding](../06-runtime/crosscut-psg-event-binding.md)
- [Module-PSG Paths](../06-runtime/module-psg-paths.md)
- [Drift Detection Spec](../06-runtime/drift-and-rollback.md)
- [Rollback Mechanisms](../06-runtime/drift-and-rollback.md)

**Cross-Cutting Concerns**:
- [AEL Deep Dive](cross-cutting/ael.md)
- [VSL Deep Dive](cross-cutting/vsl.md)
- [Orchestration](cross-cutting/orchestration.md)
- [Transaction](cross-cutting/transaction.md)
