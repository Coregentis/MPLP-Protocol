...
doc_type: normative
trace:
  schema: schemas/v2/integration/
title: L4 Integration Infrastructure
description: L4 optional integration layer for external systems including IDE, Git, CI/CD, and tool integration. Defines 4 event schemas and 17 invariant rules for MPLP interoperability.
keywords: [MPLP, Multi-Agent Lifecycle Protocol, Agent OS Protocol, AI Agent, Observable, Governed, Vendor-neutral, L4 Integration, IDE integration, Git events, CI/CD, tool events, file update events, external systems, MPLP integration]
...
The **L4 Integration Infrastructure** layer defines standard interfaces for external systems to interact with MPLP runtimes. L4 is strictly **OPTIONAL**mplementations can achieve v1.0 compliance without any L4 support. However, when integration IS provided, it MUST conform to the L4 schemas to ensure interoperability.

L4 enables:
- **IDE Integration**: File change notifications from VS Code, JetBrains, etc.
- **Version Control Integration**: Git operations (commits, pushes, merges, tags)
- **CI/CD Integration**: Build/test status from GitHub Actions, Jenkins, CircleCI
- **Tool Integration**: Linter, formatter, test runner invocations

L4 provides **one-way event flow** (External Systems MPLP Runtime) via structured events that are ingested into the Project Semantic Graph (PSG).
...
**Integration Details**:
- [07-integration/integration-spec.md](../07-integration/integration-spec.md)
