...
description: L1 Core Protocol layer specification defining 29 JSON Schemas, 59
  invariant rules, type definitions, and validation requirements for MPLP v1.0
  compliance.
keywords:
  - L1 Core Protocol
  - JSON Schema
...
spec_level: L1
normative_id: MPLP-L1-CORE
permalink: /architecture/l1-core-protocol
modules:
  - core
normative_refs: []
protocol_alignment:
  ...
  schema_refs:
    - schema_id: https://schemas.mplp.dev/v1.0/mplp-core.schema.json
      binding: manual
  invariant_refs:
    - local_path: tests/golden/invariants/core-protocol.yaml
      binding: manual
  golden_refs: []
  code_refs:
    ts: []
...
L1 is purely **declarative** and **normative**. It prescribes *WHAT* valid data looks like, but not *HOW* it is created, stored, or processed. L1 schemas and invariants are **frozen** as of 2025-12-03 and cannot be modified without incrementing to v1.1.
...
- `created_by`: Creator string (user ID, agent name, system ID)
- `updated_at`, `updated_by`: Last modification info
- `tags[]`: String array for indexing/search (uniqueItems: true)
- **`cross_cutting[]`**: **9 Cross-Cutting Concerns Enum** (critical for compliance)

**9 Cross-Cutting Concerns** (from `meta.cross_cutting` enum):
...
#### 3.2.3 trace-base.schema.json (1,859 bytes)
**Purpose**: W3C Trace Context compatible span structure

**Required Fields**:
- `trace_id`: UUID v4 - Identifies complete execution chain
- `span_id`: UUID v4 - Identifies current operation
...
L1 enforces formal validation rules stored in `schemas/v2/invariants/` as YAML files. These rules are **normative** and MUST be validated by compliant implementations.
...
- **JSON Schema Draft-07** compliant validator
...
**Module Specs** (L2):
- [Modules Overview](../02-modules/context-module.md) - All 10 module lifecycle documents
