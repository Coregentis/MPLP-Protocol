...
description: Normative invariants for MPLP Observability events. Defines runtime
  validation rules for event structure, pipeline stages, graph updates, and
  runtime execution.
keywords:
...
normative_id: MPLP-OBS-INVARIANTS
permalink: /observability/observability-invariants
cross_cutting:
  - observability
normative_refs: []
protocol_alignment:
  ...
  schema_refs: []
  invariant_refs:
    - local_path: tests/golden/invariants/observability.yaml
      binding: manual
  golden_refs: []
  code_refs:
    ts: []
