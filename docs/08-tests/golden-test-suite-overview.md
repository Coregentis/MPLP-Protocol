...
- It validates all protocol JSON schemas without errors
- It passes FLOW-01~05 in a cross-language test harness
- It preserves `agent_role` semantics for executor polymorphism

**Reference Compliance Guide**: See [`docs/02-guides/mplp-v1.0-compliance-guide.md`](../02-guides/mplp-v1.0-compliance-guide.md)

---

## 1.3 L2 Module Behavioral Invariants (v1.0 Status)

The following table shows which L2 modules have **behavioral invariants locked** vs. **structure-only definitions**:

| L2 Module  | v1.0 Status | Flows Locking Behavior | Notes |
|------------|-------------|------------------------|-------|
| **Context** | ✅ **Behavioral invariants locked** | FLOW-01~05 | Immutable problem-domain anchor, ID binding |
| **Plan** | ✅ **Behavioral invariants locked** | FLOW-01~05 | Step structure, `agent_role` polymorphism, status enum |
| **Confirm** | ✅ **Behavioral invariants locked** | FLOW-05 | Multi-round approval via `decisions[]` array |
| **Trace** | ✅ **Behavioral invariants locked** | FLOW-05 | Event logging, cross-module ID binding |
| **Role** | 🔓 Structure defined, behavior open | (future) | Role semantics vendor-specific |
| **Dialog** | 🔓 Structure defined, behavior open | (future) | Conversation threading vendor-specific |
| **Collab** | 🔓 Structure defined, behavior open | (future) | Multi-agent topology vendor-specific |
| **Extension** | 🔓 Structure defined, behavior open | (future) | Plugin system vendor-specific |
| **Core** | 🔓 Structure defined, behavior open | (future) | Governance vendor-specific |
| **Network** | 🔓 Structure defined, behavior open | (future) | Transport metadata vendor-specific |

**Design Intent**:  
> FLOW-01~05 lock down the **non-negotiable protocol semantics** (Context, Plan, Confirm, Trace).  
> Other modules provide **structural scaffolding** for product-level features.

---

## Flow Categories (A-E)

### Category A: Single Agent Basics (FLOW-01 ~ 05)
Foundation scenarios for single-agent workflows.

### Category B: Multi-Agent Collaboration (FLOW-06 ~ 10)
Multi-role coordination, dialog, and collaborative planning.

### Category C: Risk Management & Security (FLOW-11 ~ 15)
Confirmation workflows, audit trails, and security constraints.

### Category D: Error Recovery & Drift (FLOW-16 ~ 20)
Runtime errors, retries, semantic drift detection, and rollback.

### Category E: Integration & Governance (FLOW-21 ~ 25)
Project import, multi-tenancy, cost control, and end-to-end integration.

---

## Complete Flow Registry (25 Flows)

| Flow ID | Name | Category | Key L2 Modules | Purpose | Status |
|---------|------|----------|----------------|---------|--------|
| **FLOW-01** | Single Agent - Happy Path | A: Single Agent | Context, Plan, Confirm, Trace | Baseline single-agent workflow: create context → generate plan → confirm → execute trace. Validates core protocol flow. | ✅ Implemented |
| **FLOW-02** | Single Agent - Large Plan | A: Single Agent | Context, Plan, Trace | Tests scalability with 20+ step plans. Validates efficient handling of large step arrays, wildcard invariants (`steps[*]`), and sparse metadata. | ✅ Implemented |
| **FLOW-03** | Single Agent - With Tools | A: Single Agent | Context, Plan | Tool integration via `agent_role` field. Validates `curl_executor`, `jq_processor` roles and tool/agent heterogeneous composition. | ✅ Implemented |
| **FLOW-04** | Single Agent - LLM Enrichment | A: Single Agent | Context, Plan | LLM integration via `agent_role` field. Validates `llm_claude`, `llm_gpt` roles and tool/LLM/agent heterogeneous composition. | ✅ Implemented |
## MAP Profile Flows (Profile-Level, Non-Mandatory)

These flows validate MAP (Multi-Agent Profile) collaboration semantics. They are **not part of the v1.0 compliance boundary** and demonstrate multi-agent coordination patterns.

| Flow ID | Name | Category | Purpose | Status |
|---------|------|----------|---------|--------|
| MAP-01 | MAP Turn-Taking Session | MAP Profile | Two-agent sequential collaboration (round_robin) | ✅ Implemented |
| MAP-02 | MAP Broadcast Fan-out | MAP Profile | Broadcast + parallel response collection (broadcast) | ✅ Implemented |
...
6. **Respect L2/L3/L4 boundaries**: Only L2 modules in "Key L2 Modules", integration dimensions separate
