# MPLP Runtime Glue Overview (L3 Glue & PSG Paths)
...
```
┌─────────────────────────────────────────┐
│ L1/L2: Schemas & Module Semantics       │
│ (Context, Plan, Confirm, Trace, etc.)   │
└──────────────┬──────────────────────────┘
               │
               ▼
┌─────────────────────────────────────────┐
│ Profiles: Execution & Collaboration     │
│ (SA: single-agent, MAP: multi-agent)    │
└──────────────┬──────────────────────────┘
               │
               ▼
┌─────────────────────────────────────────┐
│ Runtime Glue (L3): PSG Update Rules     │ ← Phase 5
│ • Module→PSG paths                      │
│ • Event emission rules                  │
│ • Learning collection triggers          │
└──────────────┬──────────────────────────┘
               │
               ▼
┌─────────────────────────────────────────┐
│ PSG: Single Source of Truth             │
│ (nodes, edges, attributes, snapshots)   │
└─────────────────────────────────────────┘
               │
...
See:
- [`drift-detection-spec.md`](drift-detection-spec.md)
- [`rollback-minimal-spec.md`](rollback-minimal-spec.md)
...
- ⚠️ Implement minimal drift detection (compare PSG snapshots)
- ⚠️ Support basic rollback (restore PSG snapshot)
- ⚠️ Emit RuntimeExecutionEvent for agent/tool/LLM calls
- ⚠️ Collect LearningSamples at recommended triggers
...
**Phase 5 Documents**:
- [Module→PSG Paths Matrix](module-psg-paths.md)
- [Crosscut→PSG & Events Binding](crosscut-psg-event-binding.md)
- [Drift Detection Minimal Spec](drift-detection-spec.md)
- [Rollback Minimal Spec](rollback-minimal-spec.md)

**Previous Phases**:
- [SA Profile](../03-profiles/mplp-sa-profile.md) (Phase 1)
- [MAP Profile](../03-profiles/mplp-map-profile.md) (Phase 2)
- [Observability Overview](../04-observability/mplp-observability-overview.md) (Phase 3)
