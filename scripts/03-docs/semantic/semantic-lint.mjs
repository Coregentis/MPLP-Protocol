...
const CONFIG = {
    ...
    allowedContextPatterns: [
        ...
        /forbidden content/i,
        /allowed content/i,
        /incorrect terminology/i,
        /\| instead of \| use \|/i,
        /\| tier 1 \| fail \|/i,
        /\| url \| expected content \| status \|/i,
        /bad:\s*/i,
        /example terms/i,
        /historical/i,
        /replacement table/i,
        /SEAL-SSOT-ALIGNMENT-2026-01-20\.md/i
    ],
    ...
    releaseScopeExcludedPrefixes: [
        ...
        "governance/audits/",
        "scripts/01-codegen/",
        "scripts/02-verification/",
        "scripts/audit/",
        "scripts/semantic/gate_pypi_set/",
        "scripts/03-docs/semantic/schemas/"
    ],
...
function getAllFiles(dir, patterns, excludes) {
    ...
    function walk(currentDir) {
        ...
        for (const item of items) {
            ...
            if (CONFIG.releaseScopeExcludedPrefixes.some(prefix => relativePath.startsWith(prefix))) {
                continue;
            }
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (lines[i].includes("https://mplp.io") && !lines[i].includes("https://www.mplp.io")) {
                ...
                const isDocumentation =
                    CONFIG.allowedContextPatterns.some(p => p.test(context)) ||
                    CONFIG.allowedContextPatterns.some(p => p.test(file.path));
                if (!isDocumentation) {
                    violations.push({
                        file: file.path,
                        line: i + 1,
