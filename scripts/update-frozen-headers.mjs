...
> **MPLP Protocol v1.0.0 — Frozen Normative Document**
> **Freeze Date**: 2025-12-03
> **Status**: FROZEN (no breaking changes permitted)
> **Document Role**: Normative (see frontmatter for doc_role)
...
const YAML_HEADER_INVARIANT = `# MPLP v1.0.0 FROZEN – Invariant Set
# © 2025 Bangshi Beijing Network Technology Limited Company – Apache-2.0
# Governance: MPLP Protocol Governance Committee (MPGC)`;

const YAML_HEADER_REGISTRY = `# MPLP v1.0.0 FROZEN – Normative Registry
# © 2025 Bangshi Beijing Network Technology Limited Company – Apache-2.0
# Governance: MPLP Protocol Governance Committee (MPGC)`;
...
// ------------------------------------------------------------
function isRegistryYaml(filePath) {
  // Keep intentionally simple and path-based for stability
  return /glossary|registry|00-index|99-meta/i.test(filePath);
}
...
function processMarkdown(filePath) {
  ...
  // Determine file type
  // Normative: docs/00-index, docs/01..07, plus normative registries under 99-meta
  const isNormative =
    /docs[\\/]0[0-7]-/.test(filePath) ||
    /docs[\\/]00-index/.test(filePath) ||
    /docs[\\/]99-meta/.test(filePath);
  ...
  const footerRegex = /[\r\n]+---[\r\n]+© 2025 (邦士（北京）网络科技有限公司|Bangshi Beijing Network Technology Limited Company)[\s\S]*?Version 2\.0\.[\r\n]*/g;
  do {
    prevContent = content;
...
function processYaml(filePath) {
  ...
  // Add new header
  // Distinguish invariant sets vs normative registries to avoid semantic pollution
  const header = isRegistryYaml(filePath) ? YAML_HEADER_REGISTRY : YAML_HEADER_INVARIANT;
  content = header + "\n\n" + content.trimStart();
  write(filePath, content);
}
