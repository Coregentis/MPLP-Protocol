 ...
 */
class LintCommand extends BaseCommand_1.BaseCommand {
    constructor(context) {
        super(context);
        this.name = 'lint';
        this.description = 'Run linting and code quality checks';
        this.aliases = ['check'];
        this.arguments = [
            {
                name: 'files',
    ...
    async getLintConfig(args) {
        ...
        const maxWarnings = parseInt(this.getOption(args, 'max-warnings', '0'), 10);
        const format = this.getOption(args, 'format', 'stylish');
        const configPath = this.getOption(args, 'config');
        const ignorePath = this.getOption(args, 'ignore-path');
        const extensions = this.getOption(args, 'ext', '.js,.jsx,.ts,.tsx');
    ...
    async runLinting(config) {
        ...
        try {
            ...
            this.error(`Failed to run linting: ${error.message}`);
            throw error;
        }
    }
    /**
    ...
    async runESLint(config) {
        try {
            ...
            this.warn(`ESLint not found: ${error.message}`);
            return 0;
        }
    }
    /**
     ...
     */
    async runTypeScriptCheck(config) {
        try {
            ...
            this.warn(`TypeScript compiler not found: ${error.message}`);
            return 0;
        }
    }
    /**
    ...
    async runPrettierCheck(config) {
        try {
            ...
            this.warn(`Prettier not found: ${error.message}`);
            return 0;
        }
    }
    /**
     ...
     */
    async findTypeScriptExecutable() {
