...
class HelpCommand extends BaseCommand_1.BaseCommand {
    constructor() {
        super(...arguments);
        this.name = 'help';
        this.description = 'Display help information for commands';
        this.aliases = ['h'];
        this.arguments = [
            {
                name: 'command',
    ...
    async showAllCommandsHelp() {
        ...
        for (const group of commandGroups) {
            ...
            for (const cmd of group.commands) {
                this.logger.log(`  ${chalk_1.default.cyan(cmd.name.padEnd(12))} ${cmd.description}`);
            }
            this.logger.newline();
        }
    ...
    async showGeneralHelp() {
        ...
        this.logger.newline();
        this.logger.log(chalk_1.default.gray('A powerful command-line interface for building multi-agent systems'));
        this.logger.newline();
        // Usage
        ...
        for (const cmd of commonCommands) {
            this.logger.log(`  ${chalk_1.default.cyan(cmd.command.padEnd(20))} ${cmd.description}`);
        }
        this.logger.newline();
        // Global options
        ...
        for (const opt of globalOptions) {
            this.logger.log(`  ${chalk_1.default.yellow(opt.option.padEnd(20))} ${opt.description}`);
        }
        this.logger.newline();
        // Examples
        ...
        this.logger.list([
            `Documentation: ${chalk_1.default.blue('https://mplp.dev/docs')}`,
            `GitHub: ${chalk_1.default.blue('https://github.com/mplp-org/mplp')}`,
            `Discord: ${chalk_1.default.blue('https://discord.gg/mplp')}`,
            `Twitter: ${chalk_1.default.blue('https://twitter.com/mplp_dev')}`
        ]);
        this.logger.newline();
        // Footer
        this.logger.info(`For detailed help on any command, run: ${chalk_1.default.cyan(`${this.context.config.name} help <command>`)}`);
        this.logger.info(`For all available commands, run: ${chalk_1.default.cyan(`${this.context.config.name} help --all`)}`);
    }
}
