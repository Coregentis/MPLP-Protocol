 ...
 */
class ExecutionEngine extends EventEmitter_1.MPLPEventManager {
    constructor() {
        super(...arguments);
        this.agents = new Map();
    ...
    async executeAgentStep(step, context, options) {
        ...
        try {
            ...
            if (typeof agent.execute === 'function') {
                const result = await agent.execute(step.action, parameters);
                return {
                    agentId: step.agentId,
                    action: step.action,
                    parameters,
            ...
            throw new types_1.StepExecutionError(`Agent ${step.agentId} does not implement required execution interface (sendMessage or execute method)`);
        }
        catch (error) {
            throw new types_1.StepExecutionError(`Failed to execute agent step: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
